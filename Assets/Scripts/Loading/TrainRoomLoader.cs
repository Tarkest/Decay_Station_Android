...
public class TrainRoomLoader : MonoBehaviour
{
    ...
    {
        HttpController.instance.GET(StaticClasses.SERVER_ADRESS + "api/user/info", InstantiateTrain);
        _loadScreen = (Instantiate(Resources.Load("UI/LoadScreens/LoadScreen")) as GameObject).transform.GetChild(0).GetComponent<Slider>(); 
    }
    ...
    private static void InstantiateTrain(string accountInfo)
    {
        ...
        {
            TrainNPCAgent instance = (Instantiate(Resources.Load("Agents/NPC/" + info.characters[i].specialization.name + "/" + info.characters[i].name), Vector3.zero, Quaternion.identity) as GameObject).GetComponent<TrainNPCAgent>();
        ...
        Destroy(_loadScreen.transform.parent);
    }
}
